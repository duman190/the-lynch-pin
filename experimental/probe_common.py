"""Shared fixtures for the AI-tier probes in tmp/: the 2026-09-17 SMH dataset, the
main.py-equivalent parser, and the X-thread preview renderer. Not part of the app."""
import os
import re
import sys
...
from engine.ai_research import LynchPinResearcher  # noqa: E402
...
def build_prompt(idx):
    return LynchPinResearcher.build_prompt(SAMPLE, GRADES, idx, RATINGS, TECH, EDGE)


def tickers():
    return [d['Ticker'] for d in SAMPLE]
...
def render_preview(raw, idx, normalize=True):
    """Print the raw reply, the narrative_gaps verdict, and the X thread main.py would build."""
    syms = tickers()
    print("\n" + "═" * 30 + " RAW REPLY " + "═" * 30)
    print(raw)

    text = LynchPinResearcher.normalize_narrative(raw, syms) if normalize else raw
    if normalize and text != raw:
        print("\n🔧 normalize_narrative rewrote the reply (headers / SENTIMENT label / brackets were off-template)")

    sentiment, replies = parse_like_main(text, syms)
    gaps = LynchPinResearcher.narrative_gaps(text, syms)
    print(f"\n🔎 narrative_gaps → {'usable ✅' if gaps is None else 'REJECTED ❌ (' + gaps + ') — main.py would retry, not post this'}")

    idx_display = IDX_DISPLAY.get(idx, idx)
    main_tweet = f"🚨 MARKET CLOSE: ${idx} #LynchPin Detector\n\n"
    if sentiment:
        main_tweet += f"🤖: {sentiment.replace(f'${idx}', idx_display).replace('$', '')}\n\n"
    ...
    footer = (f"In this market, you'll miss the best compounders waiting for a perfect 1.0 PEG."
              ...
              f"@grok Search recent X posts about the ${idx} tickers in this thread. "
    ...
    print(footer)
    print("--------------------------")
    print(f"\n{'✅' if ok == len(syms) and sentiment else '⚠️ '} {ok}/{len(syms)} ticker replies parsed, "
